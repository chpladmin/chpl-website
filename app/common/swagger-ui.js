...
angular.module('templates/endpoint.html', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('templates/endpoint.html',
    '<div class="clearfix"> <ul class="list-inline pull-left endpoint-heading"> <li> <h4> <a href="javascript:;" ng-click="api.open=!api.open;permalink(api.open?api.name:null)" aria-expanded="{{api.open}}" ng-bind="api.name"></a> <span ng-if="api.description"> : <span ng-bind="api.description"></span></span> </h4> </li> </ul> <ul class="list-inline pull-right endpoint-actions"> <li> <a href="javascript:;" ng-click="api.open=!api.open;permalink(api.open?api.name:null)" aria-expanded="{{api.open}}">open/hide<span class="sr-only"> {{api.description}} operations</span></a> </li> <li> <a href="javascript:;" ng-click="expand(api);permalink(api.name)">list operations<span class="sr-only"> for {{api.description}}</span></a> </li> <li> <a href="javascript:;" ng-click="expand(api,true);permalink(api.name+\'*\')">expand operations<span class="sr-only"> for {{api.description}}</span></a> </li> </ul> </div> <ul class="list-unstyled collapse operations" aria-expanded="{{api.open}}"ng-class="{in:api.open}"> <li ng-repeat="op in api.operations track by $index" class="operation {{op.httpMethod}}" ng-include="\'templates/operation.html\'"></li> </ul>');
}]);

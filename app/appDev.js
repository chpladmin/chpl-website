;(function () {
    ...
    function makeFakeCerts() {
        ...
        for (var i = 0; i < allCerts[3].certs.length; i++) {
            cert = allCerts[3].certs[i];
            isActive = [true,false][Math.floor(Math.random() * 2)];
            cert.isActive = isActive;
            if (isActive) {
                cert.version = 'v' + Math.floor(Math.random() * 5);
            }
            fakeCerts[3].certs.push(cert);
        }
        return fakeCerts;
    }
    makeFakeCerts();

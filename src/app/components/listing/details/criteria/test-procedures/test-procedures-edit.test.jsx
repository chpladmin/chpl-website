...
describe('the ChplTestProceduresEdit component', () => {
  afterEach(() => {
    cleanup();
  });
  ...
  describe('when rendering', () => {
    beforeEach(async () => {
      render(
        <ChplTestProceduresEdit
          testProcedures={[
            { testProcedure: { name: 'zz name', id: 2 }, id: 2, testProcedureVersion: 'ver z' },
            { testProcedure: { name: 'name 1', id: 3 }, id: 3, testProcedureVersion: 'ver n' },
          ]}
          options={[
            { name: 'zz name', id: 2 },
    ...
    it('should sort the selected test procedures by name', async () => {
    ...
    it('should remove selected items from the list available to add', async () => {
      ...
      userEvent.click(screen.getByRole('button', { name: /Procedure Tested/i }));
  ...
  describe('when selecting items', () => {
    beforeEach(async () => {
      render(
        <ChplTestProceduresEdit
          testProcedures={[
            { testProcedure: { name: 'zz name' }, id: 2, testProcedureVersion: 'ver z' },
            { testProcedure: { name: 'name 1' }, id: 3, testProcedureVersion: 'ver n' },
          ]}
          options={[
            { name: 'zz name', id: 2 },
            { name: 'extra name', id: 6 },
    ...
    it('should call the callback', async () => {
      ...
      await waitFor(() => {
        expect(hocMock.onChange).toHaveBeenCalledWith({
          data: [
            { id: 3, testProcedure: { name: 'name 1' }, testProcedureVersion: 'ver n' },
            { id: 2, testProcedure: { name: 'zz name' }, testProcedureVersion: 'ver z' },
            { key: expect.any(Number), testProcedure: { name: 'a name', id: 4 }, testProcedureVersion: 'new version' },
          ],
          key: 'testProcedures',
        });
      });

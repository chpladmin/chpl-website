export class UtilService {
    constructor ($filter, $log, Blob, FileSaver) {
        'ngInject';
        this.$filter = $filter;
        this.$log = $log;
        this.Blob = Blob;
    ...
    makeCsv (data) {
        var blob = new this.Blob([this.arrayToCsv(data.values)], {
    ...
    sortNonconformityTypes (type) {
        if (type.name === 'Other Non-Conformity') {
            return Number.MAX_VALUE;
        }
        return this.sortCert(type);
    }

    sortOtherNonconformityTypes (type) {

...
const compare = (before, after, key, title = 'unknown') => {
  ...
  switch (key) {
    ...
    case 'certificationEvents':
      options = {
        sort: (p, c) => (p.eventDate ? (p.eventDate - c.eventDate) : (p.eventDay < c.eventDay ? -1 : (p.eventDay > c.eventDay ? 1 : 0))),
    ...
    case 'cqmResults':
      options = {
        sort: sortCqms,
        write: (f) => `CQM "${f.cmsId ? f.cmsId : (f.nqfNumber ? `NQF-${f.nqfNumber}` : 'unknown')}"`,
      };
      break;
...
const briefLookup = {
  ...
  'surveillance.randomizedSitesUsed': { message: () => undefined },
  ...
  'surveillance.type': { message: () => undefined },
  'surveillance.type.id': { message: () => undefined },
  'surveillance.type.name': { message: () => undefined },
...
const lookup = {
  ...
  'surveillance.randomizedSitesUsed': { message: (before, after) => comparePrimitive(before, after, 'randomizedSitesUsed', 'Randomized Sites Used') },
  ...
  'surveillance.type': { message: () => 'Surveillance Type' },
  'surveillance.type.name': { message: (before, after) => comparePrimitive(before, after, 'name', 'Name') },

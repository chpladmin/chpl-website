...
function authInterceptor($log, API, authService, toaster) {
  ...
  return {
    ...
    response(response) {
      ...
      if (response.data && response.data.error === 'Invalid authentication token.' && authService.hasAnyRole(['chpl-admin', 'ROLE_ONC', 'chpl-onc-acb', 'ROLE_CMS_STAFF', 'chpl-developer'])) {
        authService.logout();
      }
      return response;
    },

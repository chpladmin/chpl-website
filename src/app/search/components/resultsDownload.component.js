export const ResultsDownloadComponent = {
    templateUrl: 'chpl.search/components/resultsDownload.html',
    bindings: {
        listings: '<',
        //columns: '<',
        maxSize: '@?',
    ...
    controller: class ResultsDownloadController {
        constructor ($log, utilService) {
            ...
            this.columns = [
                { display: 'Edition', key: 'edition', enabled: true },
                { display: 'Developer', key: 'developer', enabled: true },
                { display: 'Product', key: 'product', enabled: true },
                { display: 'Version', key: 'version', enabled: true },
                { display: 'Certification Date', key: 'certificationDate', enabled: true },
                { display: 'CHPL ID', key: 'chplProductNumber', enabled: true },
                { display: 'Status', key: 'certificationStatus', enabled: true },
                { display: 'Details', key: 'id', enabled: true },
            ];
        }
        ...
        $onChanges (changes) {
            ...
            //if (changes.columns) {
            //this.columns = angular.copy(changes.columns.currentValue);
            //}
            if (changes.maxSize) {
        ...
        makeCsv () {
            this.csvData.values = [this.columns.filter(c => c.enabled).map(c => c.display)];
            this.listings.forEach(l => {
                let row = this.columns.filter(c => c.enabled).map(c => {
                    if (c.transform) {
                        return c.transform(l[c.key]);
                    } else {
                        return l[c.key]
                    }
                });
